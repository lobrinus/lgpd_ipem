...
try:
    ...
    if not auth:
        st.error("Falha na inicialização do Pyrebase 'auth'. Verifique as chaves em firebaseConfig.")
    if not storage:
        st.error("Falha na inicialização do Pyrebase 'storage'. Verifique as chaves em firebaseConfig.")
except Exception as e_pyrebase:
    st.error(f"Falha ao inicializar Pyrebase (Auth/Storage). Exceção: {e_pyrebase}")
...
if not firebase_admin._apps:
    try:
        ...
        if missing_admin_keys:
            raise KeyError(f"Chave(s) ausente(s) em FIREBASE_CREDENTIALS nos segredos: {', '.join(missing_admin_keys)}")
        ...
        st.error(f"Chave não encontrada ou ausente em 'FIREBASE_CREDENTIALS' nos 'Secrets' do Streamlit Cloud: {e_key_admin}.")
    except ValueError as e_value_admin:
        st.error(f"Formato inválido para 'FIREBASE_CREDENTIALS' nos 'Secrets' do Streamlit Cloud: {e_value_admin}")
